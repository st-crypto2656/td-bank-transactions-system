...
public:
	...
	void deposit(int i)
	{
		cout << "\nHello Customer #" << i + 1 << ", how much would you like to deposit into account name: " << name[i] << " with a balance of: " << accBal[i] << "\n";
		cin >> dep[i];
		accBal[i] += dep[i];
	}
	...
	void withdraw(int i)
	{
		cout << "\nHello Customer #" << i + 1 << ", how much would you like to withdraw from account name: " << name[i] << " with a balance of: " << accBal[i] << "\n";
		cin >> extract[i];
		if (extract[i] > accBal[i])
		{
			cout << "Insufficient funds, withdrawal was unsuccesful!\n";
		}
		else
		{
			accBal[i] -= extract[i];
	...
	void showMenu(int i)
	{
		...
		do
		{
			...
			case 4:
				system("clear");
				cout << "Customer #" << i + 1 << ". Thank you for using TD◍.\n";
				break;
			default:
...
int main()
{
	...
	cout << "Thank you for using TD◍. Goodbye!\n";
